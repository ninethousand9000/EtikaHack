package me.ninethousand.etikahack.api.config;
...
import me.ninethousand.etikahack.api.util.misc.EnumUtil;
...
public class Config {
    ...
    public static void saveFriends() throws IOException {
        ...
        OutputStreamWriter fileOutputStreamWriter = new OutputStreamWriter(new FileOutputStream("etikahack/friends.json"), StandardCharsets.UTF_8);
        JsonObject friendObject = new JsonObject();

        for (String friend : FriendManager.getFriends()) {
            friendObject.add("Name", new JsonPrimitive(friend));
        }

        String jsonString = gson.toJson(new JsonParser().parse(friendObject.toString()));
